...
def knives_image_upload_to(instance, filename):
    return f'knives/{instance.item_id}/images/{filename}'


class KnifeImageType(models.TextChoices):
    FRONT = 'front', 'رو'
    BACK = 'back', 'پشت'
...
class KnifeImage(models.Model):
    item = models.ForeignKey(
        'knives.Knife', on_delete=models.CASCADE, related_name='images', verbose_name='چاقو'
    )
    image = models.ImageField(upload_to=knives_image_upload_to, verbose_name='تصویر')
    image_type = models.CharField(
        max_length=20, choices=KnifeImageType.choices, default=KnifeImageType.OTHER, verbose_name='نوع تصویر'
    )
    ...
    uploaded_by = models.ForeignKey(
        ...
        related_name='uploaded_knives_images', verbose_name='آپلودکننده',
    )
...
class KnifePurchaseRecord(models.Model):
    item = models.ForeignKey(
        'knives.Knife', on_delete=models.CASCADE, related_name='purchase_records', verbose_name='چاقو'
    )
    ...
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True,
        related_name='created_knives_purchases', verbose_name='ایجادکننده',
    )
    ...
    def __str__(self):
        return f'Purchase {self.pk} for knives {self.item_id}'


class KnifeValuationRecord(models.Model):
    item = models.ForeignKey(
        'knives.Knife', on_delete=models.CASCADE, related_name='valuation_records', verbose_name='چاقو'
    )
    ...
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True,
        related_name='created_knives_valuations', verbose_name='ایجادکننده',
    )
    ...
    def __str__(self):
        return f'Valuation {self.pk} for knives {self.item_id}'
