...
def tasbih_image_upload_to(instance, filename):
    return f'tasbih/{instance.item_id}/images/{filename}'


class TasbihImageType(models.TextChoices):
    FRONT = 'front', 'رو'
    BACK = 'back', 'پشت'
...
class TasbihImage(models.Model):
    item = models.ForeignKey(
        'tasbih.Tasbih', on_delete=models.CASCADE, related_name='images', verbose_name='تسبیح'
    )
    image = models.ImageField(upload_to=tasbih_image_upload_to, verbose_name='تصویر')
    image_type = models.CharField(
        max_length=20, choices=TasbihImageType.choices, default=TasbihImageType.OTHER, verbose_name='نوع تصویر'
    )
    ...
    uploaded_by = models.ForeignKey(
        ...
        related_name='uploaded_tasbih_images', verbose_name='آپلودکننده',
    )
...
class TasbihPurchaseRecord(models.Model):
    item = models.ForeignKey(
        'tasbih.Tasbih', on_delete=models.CASCADE, related_name='purchase_records', verbose_name='تسبیح'
    )
    ...
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True,
        related_name='created_tasbih_purchases', verbose_name='ایجادکننده',
    )
    ...
    def __str__(self):
        return f'Purchase {self.pk} for tasbih {self.item_id}'


class TasbihValuationRecord(models.Model):
    item = models.ForeignKey(
        'tasbih.Tasbih', on_delete=models.CASCADE, related_name='valuation_records', verbose_name='تسبیح'
    )
    ...
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True,
        related_name='created_tasbih_valuations', verbose_name='ایجادکننده',
    )
    ...
    def __str__(self):
        return f'Valuation {self.pk} for tasbih {self.item_id}'
