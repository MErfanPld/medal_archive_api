...
def stamps_image_upload_to(instance, filename):
    return f'stamps/{instance.item_id}/images/{filename}'


class StampImageType(models.TextChoices):
    FRONT = 'front', 'رو'
    BACK = 'back', 'پشت'
...
class StampImage(models.Model):
    item = models.ForeignKey(
        'stamps.Stamp', on_delete=models.CASCADE, related_name='images', verbose_name='تمبر'
    )
    image = models.ImageField(upload_to=stamps_image_upload_to, verbose_name='تصویر')
    image_type = models.CharField(
        max_length=20, choices=StampImageType.choices, default=StampImageType.OTHER, verbose_name='نوع تصویر'
    )
    ...
    uploaded_by = models.ForeignKey(
        ...
        related_name='uploaded_stamps_images', verbose_name='آپلودکننده',
    )
...
class StampPurchaseRecord(models.Model):
    item = models.ForeignKey(
        'stamps.Stamp', on_delete=models.CASCADE, related_name='purchase_records', verbose_name='تمبر'
    )
    ...
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True,
        related_name='created_stamps_purchases', verbose_name='ایجادکننده',
    )
    ...
    def __str__(self):
        return f'Purchase {self.pk} for stamps {self.item_id}'


class StampValuationRecord(models.Model):
    item = models.ForeignKey(
        'stamps.Stamp', on_delete=models.CASCADE, related_name='valuation_records', verbose_name='تمبر'
    )
    ...
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, blank=True,
        related_name='created_stamps_valuations', verbose_name='ایجادکننده',
    )
    ...
    def __str__(self):
        return f'Valuation {self.pk} for stamps {self.item_id}'
