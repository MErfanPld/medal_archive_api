...
def validate_coin_image(uploaded_file):
    max_bytes = getattr(settings, 'COIN_IMAGE_MAX_BYTES', 10 * 1024 * 1024)
    ...
    if _ext(name) not in ALLOWED_IMAGE_EXTENSIONS:
        raise ValidationError(
            f'پسوند تصویر نامعتبر است. مجاز: {", ".join(sorted(ALLOWED_IMAGE_EXTENSIONS))}'
        )
